...
#1пошук за параметрами
year, volume, price = search_criteria

filtered_cars = []
for car_name, car_info in car_data.items():
  car_colour, car_year, car_volume, car_type, car_price = car_info
  if car_year >= year and car_volume >= volume and car_price <= price:
    filtered_cars.append((car_name, car_info))
print(filtered_cars)

#сортування за зростанням ціни
def get_car_price(car):
  return car[1][4]
filtered_cars = sorted(filtered_cars, key=get_car_price)
print(filtered_cars)

#виведення перший 5 значень

print(filtered_cars[0:5])

for i, (car_name_1, car_info_1) in enumerate(filtered_cars[:5], 1):
    car_colour_1, car_year_1, car_volume_1, car_type_1, car_price_1 = car_info_1
    print(f"{i}. {car_name_1} -  Ціна: {car_price_1:,}$, Рік: {car_year_1}, Об'єм: {car_volume_1}L, Тип: {car_type_1}")